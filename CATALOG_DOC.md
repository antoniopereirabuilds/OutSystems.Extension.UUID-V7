...
- goldenThreadId is sanitized before propagation: it is capped at 200 characters and stripped of control characters (C0 range and DEL) to prevent log-line forging in downstream consumers.
- Do not pass PII, secrets, or session tokens in goldenThreadId. The value is persisted in plain text inside flightPath; if you store flightPath rows, apply a retention policy that aligns with your data-protection obligations.
